...
def train(x_train,y_train,x_test, y_test,Validata_Percentage):
    """定义训练过程"""
    ...
    with tf.Session() as sess:
        ...
        # 训练过程中的验证集——————此步需要改进
        Validata_size = int(Validata_Percentage*len(y_train))
        validate_feed = {x: x_train[0:Validata_size], y_: y_train[0:Validata_size]}
        # 训练集
        x_trainSet = x_train[Validata_size:-1]
        y_trainSet = y_train[Validata_size:-1]
        # 测试集
        ...
        for i in range(TRAINING_STEPS):
            ...
            #遍历完所有的训练数据所需要的迭代次数
            Iteration_data=int(int(len(y_train)*(1-Validata_Percentage)) / BATCH_SIZE)
            xs, ys = get_batch(x_trainSet, y_trainSet, BATCH_SIZE, i % Iteration_data, Iteration_data)
